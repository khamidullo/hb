package uz.hbs.actions.touragent.newbooking.group;
...
public class TAGroupReservationPanel extends MyBreadCrumbPanel {
	private static final Logger _log = LoggerFactory.getLogger(TAGroupReservationPanel.class); 
	...
	public TAGroupReservationPanel(String id, IBreadCrumbModel breadCrumbModel, IModel<GroupReservation> model, final Booking filter) {
		this(id, breadCrumbModel, model, filter, true);
	}
		
	public TAGroupReservationPanel(String id, IBreadCrumbModel breadCrumbModel, IModel<GroupReservation> model, final Booking filter, final boolean isEditable) {		
		super(id, breadCrumbModel, model);
		
		add(dialog = new ModalWindow("dialog"));
		
		
		final GroupReservation reserve = model.getObject();
		...
		final Form<GroupReservation> reserveform;
		final Button done;
		...
		reserveform.add(done = new Button("ready") {
			...
			public void onSubmit() {
				final GroupReservation reserve = (GroupReservation) reserveform.getDefaultModelObject();
				activate(new IBreadCrumbPanelFactory(){
					private static final long serialVersionUID = 1L;
		...
		additional_option.setVisible((Boolean) new MyBatisHelper().selectOne("selectAdditionalServiceTashkent", model.getObject().getHotelsusers_id()));
	...
	private class ReserveForm extends Form<GroupReservation> {
		...
		public ReserveForm(String id, final GroupReservation reserve, final ReservationRuleType rule) {
			...
			add(new RequiredTextField<>("group_name").setLabel(new StringResourceModel("hotels.reservation.details.group.name", null)));
			...
			container.add(new ListView<ReservationRoom>("reserveroomlist", new LoadableDetachableModel<List<ReservationRoom>>() {
				...
				protected void populateItem(final ListItem<ReservationRoom> item) {
					...
					reserve_edit.add(roomtype = new DropDownChoice<RoomType>("roomtype", new PropertyModel<RoomType>(item.getModel(), "roomtype"), new LoadableDetachableModel<List<RoomType>>() {
						private static final long serialVersionUID = 1L;
						...
						protected boolean isDisabled(RoomType object, int index, String selected) {
							if (item.getModelObject().getId() == null && ! selected.equals(String.valueOf(object.getId()))) return true;
							return super.isDisabled(object, index, selected);
						};
					});
					if (reserve.getId() == null) roomtype.add(new AttributeModifier("readonly", true));
...
//								};
					...
					item.add(listview = new ListView<Guest>("guests", new LoadableDetachableModel<List<Guest>>() {
						...
						protected void populateItem(final ListItem<Guest> guestItem) {
							guestItem.add(new GuestDataPanel("guestdetails", new Model<Guest>(guestItem.getModelObject()), item.getModelObject(), guestItem.getIndex(), rule.getMinimum_free_age(), item.getIndex(), reserve.isResident()){
								private static final long serialVersionUID = 1L;
								...
								protected void onDelete(AjaxRequestTarget target) {
									...
									} else item.getModelObject().getGuestlist().remove(guestItem.getIndex());
									target.add(amountOfGuestsLabel);
									ReserveUtil.recalcTAIndividualTotalAmount(reserve, rule);
								...
								protected boolean isVisibleAppend() {
									short roomtype_holding_capacity = new MyBatisHelper().selectOne("selectHoldingCapacityRoomType", item.getModelObject().getRoomtype().getId());
									return roomtype_holding_capacity > (short)(item.getModelObject().getAdults_count() - (item.getModelObject().isExtra_bed_needed()?1:0));
								}
					...
					roomtype.add(new AjaxFormComponentUpdatingBehavior("onchange") {
						...
						protected void onUpdate(AjaxRequestTarget target) {
							try {
								...
								if (count == 0) {
									feedback.error(new StringResourceModel("hotels.reservation.room.vacant.not.found", null, new Object[] { item
											.getModelObject().getRoomtype().getName() }).getString());
									...
									if (item.getModelObject().getGuestlist().size() < holding_capacity) {
										...
										for (short guest = (short) (item.getModelObject().getGuestlist().size() + 1); guest <= holding_capacity; guest++) {
			...
			add(new AjaxMealOptionCallBackOther() {
				...
				protected void onUpdate(AjaxRequestTarget target, String objId, byte objVal, short guest_count) {
					...
					if (temp > 0) {
						if (temp == 3) {
							Double d1 = CommonUtil.nvl((Double) new MyBatisHelper().selectOne("selectTAMealOptionValue", new MealOption(MealOption.LUNCH, reserve.getHotelsusers_id())));
							Double d2 = CommonUtil.nvl((Double) new MyBatisHelper().selectOne("selectTAMealOptionValue", new MealOption(MealOption.DINNER, reserve.getHotelsusers_id())));
							...
							d =  1d * ReserveUtil.calcMealOption(reserve.getCheck_in(), reserve.getCheck_out(), temp, CommonUtil.nvl((Double) new MyBatisHelper().selectOne("selectTAMealOptionValue", new MealOption(temp, reserve.getHotelsusers_id()))));
						}
			...
			add(new AjaxCheckPeriodCallBack() {
				...
				protected void onUpdate(AjaxRequestTarget target, String check_in, String check_out, Short number_of_nights) {
					...
					try {
						...
						if (number_of_nights != null) reserve.setNumber_of_nights(number_of_nights); 
//						reserve.setTotal(ReserveUtil.getTAIndividualTotalAmount(reserve, rule));
						...
						if (reserve.getId() != null) {
							...
							} else reserve.newlyReserve = false;
						}
	...
	private abstract class GuestDataPanel extends Panel {
		...
		public GuestDataPanel(String id, final Model<Guest> model, final ReservationRoom reserveroom, final int guest_index, final Short free_age, final int reserveroom_index, final boolean resident) {
			super(id, model);
			...
			add(person_title = new DropDownChoice<PersonTitle>("person_title",  new PropertyModel<PersonTitle>(model, "person_title"), PersonTitleModel.getPersonTitle(), new ChoiceRenderer<PersonTitle>("name", "title")){
				private static final long serialVersionUID = 1L;

				@Override
				protected boolean isDisabled(PersonTitle object, int index, String selected) {
					if (guest_index == 0 && object.getTitle().equals(PersonTitle.CHILD)) return true;
					if (reserveroom.getChildAgeList().isEmpty() && object.getTitle().equals(PersonTitle.CHILD)) return true;
					return super.isDisabled(object, index, selected);
				}
			});
			person_title.setNullValid(true);
			person_title.setRequired(require);
			if (require) person_title.add(new AttributeModifier("required", require));
			...
			if (require) first_name.add(new AttributeModifier("required", require));
			...
			if (require) last_name.add(new AttributeModifier("required", require));
			...
			if (require) nationality.add(new AttributeModifier("required", require));
			...
			add(guestAge = new DropDownChoice<ChildAge>("childAge", new PropertyModel<ChildAge>(model, "childAge"), new LoadableDetachableModel<List<ChildAge>>() {
				private static final long serialVersionUID = 1L;
				...
				public Object getDisplayValue(ChildAge object) {
					if (object.getChild_index() == ChildAge.UNKNOWN) return new StringResourceModel("hotels.guest.details.child.age", null).getString();
				...
				protected boolean isDisabled(ChildAge object, int index, String selected) {
					if (selectedAgesList.contains(object.getChild_index())) {
						if (! "".equals(CommonUtil.nvl(selected)) && object.getChild_index() == Short.valueOf(selected)) return false;
						return true;
					}
					...
					if (object.getAge() <= free_age && object.getAge() >= 0) return true;
					return super.isDisabled(object, index, selected);
				}
			...
			guestAge.setVisible(model.getObject().getChildAge().getChild_index() != ChildAge.UNKNOWN || (model.getObject().getPerson_title() != null && model.getObject().getPerson_title().getTitle().equals(PersonTitle.CHILD)));
	...
	private abstract class AddRoomTypePanel extends Panel {
		...
		public AddRoomTypePanel(String id, final GroupReservation reserve) {
			super(id);
			add(feedback = new MyFeedbackPanel("feedback"));
			feedback.setOutputMarkupId(true);
				...
				protected void onUpdate(AjaxRequestTarget target) {
					...
					try {
						...
						if (roomtype.getId() != null) {
							...
							if (count == 0) feedback.error(new StringResourceModel("hotels.reservation.room.vacant.not.found", null, new Object[] { roomtype.getName() }).getString());
						}
						target.add(select);
					} finally {
						target.add(feedback);
