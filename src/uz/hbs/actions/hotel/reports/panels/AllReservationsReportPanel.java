...
public class AllReservationsReportPanel extends ReservationReportPanel {
	private static final long serialVersionUID = 1L;

	public AllReservationsReportPanel(String id, IBreadCrumbModel breadCrumbModel, final ValueMap model) {
		super(id, breadCrumbModel, model);

		model.put("hotel_id", ((MySession) getSession()).getUser().getHotelsusers_id());
		final List<HotelReservationReport> list = new MyBatisHelper().selectList("selectAllReservationsReport", model); 
		...
		add(new ListView<HotelReservationReport>("reportlist", reportListModel) {
			...
			protected void populateItem(ListItem<HotelReservationReport> item) {
				...
				item.add(new Label("create_date", FormatUtil.toString(report.getCreate_date(), MyWebApplication.DATE_TIME_FORMAT)));
				item.add(new Label("guest_name", report.getFirst_name() + " " + report.getLast_name()));
				item.add(new Label("tour_agent", report.getTour_agent()));
				item.add(new Label("check_in", FormatUtil.toString(report.getCheck_in(), MyWebApplication.DATE_TIME_FORMAT)));
				item.add(new Label("check_out", FormatUtil.toString(report.getCheck_out(), MyWebApplication.DATE_TIME_FORMAT)));
					...
					public String getObject() {
						switch (report.getStatus()) {
							...
							case ReservationStatus.CHECKED_IN:
								return  getString("hotels.reservation.details.reservation.status.active");
		...
		final AJAXDownload download = new AJAXDownload() {
				...
				IResourceStream resourceStream = new AbstractResourceStreamWriter() {
					...
					public void write(OutputStream os) throws IOException {
						...
						try {
							...
							Sheet s = wb.createSheet("AllReservations");
					        int _col = 0;
					        int _row = 0;
			...
			protected String getFileName() {
				return "AllReservations_" + FormatUtil.toString(new Date(), MyWebApplication.DATE_FORMAT) + ".xlsx";
			}
		};
		add(download);
