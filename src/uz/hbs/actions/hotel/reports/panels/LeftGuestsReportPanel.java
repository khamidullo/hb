...
public class LeftGuestsReportPanel extends ReservationReportPanel {
	private static final long serialVersionUID = 1L;

	public LeftGuestsReportPanel(String id, IBreadCrumbModel breadCrumbModel, final ValueMap model) {
		super(id, breadCrumbModel, model);
		
		model.put("hotelsusers_id", ((MySession) getSession()).getUser().getHotelsusers_id());
		final List<HotelReservationReport> list = new MyBatisHelper().selectList("selectLeftGuestReport", model); 
		...
		final AJAXDownload download = new AJAXDownload() {
			...
			protected String getFileName() {
				return "LeftGuests_" + FormatUtil.toString(new Date(), MyWebApplication.DATE_FORMAT) + ".xlsx";
			}
		};
		add(download);
