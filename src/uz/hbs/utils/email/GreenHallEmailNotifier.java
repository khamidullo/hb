...
public class GreenHallEmailNotifier {
	private static final Logger logger = LoggerFactory.getLogger(GreenHallEmailNotifier.class);
	...
	private static void prepareDataFromDB(long additionalServiceOrderId, boolean isCancel) {
		...
		param.put("air_service_type", AdditionalServiceDetail.AIR_SERVICE_TYPE_GREEN_HALL);
		...
		if ((Boolean) new MyBatisHelper().selectOne("selectIsExistsServiceType", param)) {
			detail = new MyBatisHelper().selectOne("selectGreenHallGuests", param);
			if (detail != null)
				prepareHtml(order, tourAgent, price, true, detail, isCancel);
		}

		param.put("service_type", AdditionalServiceDetail.SERVICE_TYPE_DEPARTED);

		if ((Boolean) new MyBatisHelper().selectOne("selectIsExistsServiceType", param)) {
			AdditionalServiceDetail detail2 = new MyBatisHelper().selectOne("selectGreenHallGuests", param);
			if (detail2 != null)
				prepareHtml(order, tourAgent, price, false, detail2, isCancel);
			else if (detail2 == null && order.getReservations_id() != null && detail != null) {
				prepareHtml(order, tourAgent, price, false, detail, isCancel);
			}
		}
	}
	...
	private static void prepareHtml(AdditionalServiceOrder order, final TourAgent tourAgent, final AdditionalServicePrice price,
			final boolean isArrival, AdditionalServiceDetail detail, boolean isCancel) {
		Map<String, Object> attributes = new HashMap<String, Object>();

		attributes.put("where", "ЦУДО НК Узбектуризм");
		attributes.put("whom", "Кому: Директору, Равшанбекову Ж.Р");
		if (isCancel) {
			attributes.put("cancel_info", "<br /><span style=\"font-size: 48px; font-weight: bold;\">ОТМЕНА</span>");
		}
		attributes.put("tour_agency", tourAgent.getName());
		attributes.put("order_id", order.getId() + "/" + tourAgent.getLicense_id());
		attributes.put("arrival_or_departure", isArrival ? "<span style=\"font-weight: bold; text-decoration: underline;\">ВСТРЕЧА</span> / ПРОВОДЫ"
				: "ВСТРЕЧА / <span style=\"font-weight: bold; text-decoration: underline;\">ПРОВОДЫ</span>");
		attributes.put("order_date_time", DateUtil.toString(order.getCreate_date(), MyWebApplication.DATE_TIME_SHORT_FORMAT));
		...
		StringTemplate st = templateGroup.getInstanceOf("uz/hbs/utils/email/templates/reservation/GreenHall.html", attributes);
		sendEmail(price.getEmail_send_request_green(), st.toString(), String.valueOf(order.getId()), creator_email);
	}
	...
	private static void sendEmail(final String recipient, final String content, final String number, String creator_email) {
		new Thread() {
			...
			public void run() {
				...
				message.setSubject(serverType + "Заявка на Green Hall №" + number);
