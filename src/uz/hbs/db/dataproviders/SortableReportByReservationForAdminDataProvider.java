...
public class SortableReportByReservationForAdminDataProvider extends SortableDataProvider<ReservationDetail, String>
		implements IFilterStateLocator<AllReservationFilter> {
	private static final long serialVersionUID = 1L;
	private static Logger _log = LoggerFactory.getLogger(SortableReportByReservationForAdminDataProvider.class);
	private AllReservationFilter filter;

	public SortableReportByReservationForAdminDataProvider() {
	...
	public Iterator<ReservationDetail> iterator(long offset, long limit) {
		...
		try {
			...
			list = new MyBatisHelper().selectList("selectReservationsReportListForAdmin", param);
	...
	public long size() {
		try {
			...
			Long count = (Long) new MyBatisHelper().selectOne("selectReservationsReportCountForAdmin", param);
