...
public class SortableAllReservationDataProvider extends SortableDataProvider<ReservationDetail, String> implements IFilterStateLocator<AllReservationFilter> {
	private static final long serialVersionUID = 1L;
	private static Logger _log = LoggerFactory.getLogger(SortableAllReservationDataProvider.class);
	private AllReservationFilter filter;
	
	public SortableAllReservationDataProvider() {
	...
	public Iterator<ReservationDetail> iterator(long offset, long limit) {
		...
		try {
			...
			list = new MyBatisHelper().selectList("selectAllReservationList", param);
	...
	public long size() {
		try {
			...
			Long count = (Long) new MyBatisHelper().selectOne("selectAllReservationListCount", param);
